...
export class ProgressCardsComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    const currentMonthRequest$ =
      this.aggregationService.getMonthAggregation(currentMonth);
    const previousMonthRequest$ =
      this.aggregationService.getMonthAggregation(previousMonth);

    combineLatest([currentMonthRequest$, previousMonthRequest$]).subscribe(
      ([current, previous]) => {
  ...
  private buildTrends(): void {
    ...
    this.hasSongAggregation =
      this.currentMonth()!.songAggregation != null &&
      this.previousMonth()!.songAggregation != null;

    this.hasRhythmAggregation =
      this.currentMonth()!.rhythmAggregation != null &&
      this.previousMonth()!.rhythmAggregation != null;

    this.hasSoloAggregation =
      this.currentMonth()!.soloAggregation != null &&
      this.previousMonth()!.soloAggregation != null;
  }
}
