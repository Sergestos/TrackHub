...
export class AggregationService {
  ...
  public getMonthAggregation(date: Date): Observable<ExerciseAggregation> {
    return this.apiService.get<ExerciseAggregation>(
      this.exercisesUrl + `?date=${date.toDateString()}`,
    );
  }

  public getMonthRangeAggregation(
    startDate: Date,
    endDate: Date,
  ): Observable<ExerciseAggregation[]> {
    const query = `?startDate=${startDate.toDateString()}&endDate=${endDate.toDateString()}`;
    return this.apiService.get<ExerciseAggregation[]>(
      this.exercisesUrl + '/range' + query,
    );
  }
